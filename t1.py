...
class YanHuaBoYi(object):
    ...
    def get_page_url(self):
        ...
        if index in range(len(data_url_temp)):
            ...
            str_list = temp.split('&')
            return f"https://kns.cnki.net/kcms/detail/detail.aspx?dbcode=CAPJ&{str_list[4]}&{str_list[5]}"
        else:
    ...
    def get_time(self):
        ...
        if data_url_temp := re.findall(find_element, html):
    ...
    def get_keyword(self):
        ...
        keywords = []
        for item in a_tags:
            if keyword := re.findall(re.compile(r"'kw','(.*?)',"), str(item)):
                keywords = keyword
        return keywords
...
if __name__ == '__main__':
    article = YanHuaBoYi(
        page=int(input("请输入页数:")), index=int(input("请输入文章序号:")), article_type=1
    )
